...
public class ContentDataSeeder implements CommandLineRunner {
    ...
    private void seedModulesAndTopics() {
        ...
        seedPassiveVoiceLessons(topPassive);
    ...
    private void seedPassiveVoiceLessons(TopicEntity topPassive) {
        UUID remedialPassiveId = UUID.fromString("55555555-0000-0000-0000-000000000008");
        if (lessonRepository.findById(remedialPassiveId).isEmpty()) {
            LessonEntity remedialPassive = LessonEntity.builder()
                    .id(remedialPassiveId)
                    .topicId(topPassive.getId())
                    .skillId(topPassive.getId())
                    .title("[Bài học cải thiện] Chuyên đề Bị động đặc biệt & Cấu trúc nhờ vả")
                    .theorySummary("Bài học bổ trợ củng cố: Nắm vững thể bị động kép, bị động với 2 tân ngữ và cấu trúc causative have/get.")
                    .content("""
                            ### 1. Bị động với động từ có 2 tân ngữ (give, send, show, buy...)
                            - **Chủ động:** S + V + O1 (người) + O2 (vật)
                            - **Bị động 1 (ưu chuộng hơn):** O1 + be + V3/ed + O2. (*I was given a nice gift.*)
                            ...
                            - **Have:** S + have + O(người) + V_inf = S + have + O(vật) + V3/ed.
                              - *I had the mechanic repair my car.* $\\rightarrow$ *I had my car repaired.*
                            - **Get:** S + get + O(người) + to V_inf = S + get + O(vật) + V3/ed.
                              - *She got her brother to fix the lamp.* $\\rightarrow$ *She got the lamp fixed.*

                            ### 4. Mẹo giải nhanh đề thi trắc nghiệm
                            - Nhìn ngay sau chỗ trống: nếu **không có tân ngữ trực tiếp** mà có giới từ (by, in, with...) $\\rightarrow$ 90% là câu **Bị động**!
                            """)
                    .displayOrder(2)
                    .isRemedial(true)
                    .status("PUBLISHED")
                    .build();
            lessonRepository.save(remedialPassive);

            miniQuizRepository.save(MiniQuizEntity.builder()
                    .lessonId(remedialPassive.getId())
                    .title("Quiz: Ôn tập Củng cố Câu bị động")
                    .description("Trả lời 3 câu hỏi củng cố kiến thức trước khi làm lại bài kiểm tra Topic.")
                    .questionsJson("""
                            [
                              {
